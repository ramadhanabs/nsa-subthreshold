import { Sun, Flame, AlertTriangle, Skull } from "lucide-react"
      ...
      {zones.map((z) => (
        <div key={z.title} className="bg-muted rounded-lg px-3.5 py-3">
          <div className="flex items-center gap-1.5 mb-1">
            <z.icon size={13} className={z.iconColor} />
            <div className="text-[0.7rem] font-medium text-muted-foreground">{z.title}</div>
          </div>
          {z.lines.map((line) => (
